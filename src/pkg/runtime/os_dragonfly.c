...
runtime·futexsleep(uint32 *addr, uint32 val, int64 ns)
{
	...
	ret = runtime·sys_umtx_sleep(addr, val, (int32) secs);
	if(ret >= 0 || ret == -EINTR)
		return;
...
static int8 badsignal2[] = "runtime: signal received on thread not created by Go: ";

// This runs on a foreign stack, without an m or a g.  No stack split.
#pragma textflag 7
void
runtime·badsignal2(int32 sig)
{
	int32 len;
	...
	runtime·write(2, badsignal2, sizeof badsignal2 - 1);
	if (0 <= sig && sig < NSIG) {
